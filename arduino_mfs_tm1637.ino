...
void loop() {
  ...
  // ---------- 7. Update layar 4-digit TM1637 sesuai mode ----------
  if (now - tDisplayUpdate >= 300) {
    ...
    if (displayMode == 0) {
      ...
      display.showNumberDec((int)tempC, true); // contoh: suhu 24C -> "0024"
    } else {
      if (hasIRCode) {
        if (irHexFormat) {
          // Tampilkan 4 digit heksa terakhir dari kode IR, contoh: 0xFFA25D -> "A25D"
          display.showNumberHexEx((uint16_t)(lastIRCode & 0xFFFF), 0, true);
        } else {
          // Tampilkan 4 digit desimal terakhir dari kode IR, contoh: 4293848669 -> "8669"
          display.showNumberDec((int)(lastIRCode % 10000UL), true);
        }
      } else {
